...
if (!createObject('modNamespace',array(
    'name' => 'commerce_pfcheckout',
    'path' => $componentPath.'/core/components/commerce_pfcheckout/',
    'assets_path' => $componentPath.'/assets/components/commerce_pfcheckout/',
),'name', false)) {
    echo "Error creating namespace commerce_pfcheckout.\n";
}

/* Path settings */
if (!createObject('modSystemSetting', array(
    'key' => 'commerce_pfcheckout.core_path',
    'value' => $componentPath.'/core/components/commerce_pfcheckout/',
    'xtype' => 'textfield',
    'namespace' => 'commerce_pfcheckout',
    'area' => 'Paths',
    'editedon' => time(),
), 'key', false)) {
    echo "Error creating commerce_pfcheckout.core_path setting.\n";
}

if (!createObject('modSystemSetting', array(
    'key' => 'commerce_pfcheckout.assets_path',
    'value' => $componentPath.'/assets/components/commerce_pfcheckout/',
    'xtype' => 'textfield',
    'namespace' => 'commerce_pfcheckout',
    'area' => 'Paths',
    'editedon' => time(),
), 'key', false)) {
    echo "Error creating commerce_pfcheckout.assets_path setting.\n";
}
...
$assetsUrl = "{$requestUri}assets/components/commerce_pfcheckout/";

if (!createObject('modSystemSetting', array(
    'key' => 'commerce_pfcheckout.assets_url',
    'value' => $assetsUrl,
    'xtype' => 'textfield',
    'namespace' => 'commerce_pfcheckout',
    'area' => 'Paths',
    ...
    echo "Error creating commerce_pfcheckout.assets_url setting.\n";
}
...
foreach ($settings as $key => $opts) {
    ...
    if (!createObject('modSystemSetting', array(
        ...
        echo "Error creating commerce_pfcheckout.".$key." setting.\n";
    }
}
...
include $componentPath . '/core/components/commerce_pfcheckout/vendor/autoload.php';

// Grab the path to our namespaced files
$modulePath = $componentPath . '/core/components/commerce_pfcheckout/src/Modules/';

// Instruct Commerce to load modules from our directory, providing the base namespace and module path twice
$commerce->loadModulesFromDirectory($modulePath, 'DigitalPenguin\\Commerce_PFCheckout\\Modules\\', $modulePath);
